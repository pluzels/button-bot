...
async function handler(m, { conn, usedPrefix, command, text }) {
  ...
  try {
    ...
    if (!(m.sender in conn.bj)) {
      ...
      if (isNaN(bet) || bet <= 0) {
        return m.reply(`• *Example :* .${command} 1000000`);
      }
      ...
      if (playerTotal === 21 && computerTotal !== 21) {
        ...
        message += '*Y O U R - C A R D S*\n'
        message += '```Kartu Kamu:```' + ` ${playerCards.join(', ')}\n`;
        message += '```Total Kamu:```' + ` ${playerTotal}\n\n`;
        message += '*B O T - C A R D S*\n'
        message += '```Kartu Komputer:```' + ` ${computerCards[0]}, ?\n`;
        message += '```Taruhan:```' + ` Rp ${toRupiah(bet)}\n\n`;
        message += Func.Styles(`Ketik *hit* untuk mengambil kartu tambahan.\n`)
        message += Func.Styles(`Ketik *stand* untuk mengakhiri giliran.`)
