...
Regards : *Kemii*`
let kemii = await conn.sendMessage(group.id, { text: capt, mentions: participants.map(a => a.id) }, {quoted:fverif})
await conn.sendContact(group.id, ([nomorown, 'Costumer - Service']), kemii)
...
conn.reply(m.chat, `• *Example :* .out nomer`, m);
}
}
handler.help = ['out']
handler.tags = ['owner']
